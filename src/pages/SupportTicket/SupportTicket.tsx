...
import { GetAllSupportTicket, GetAllSupportTicketByTicketNumber } from "./SupportTicketSlice";
...
const SupportTicket = () => {
    ...
    const {SupportTicketData, isLoading} = useSelector((state: RootState) => state.SupportTicketSlice)
  ...
  setSearchByTicket(true); 
    ...
    // 📝  ADD / EDIT mode (your existing logic)
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {paginatedUsers?.length > 0 ? (
            paginatedUsers.map((ticket, index) => (
              <tr key={ticket.id} className="hover:bg-gray-50 transition-all">
                ...
                <td className={TableDataClass}>
                  <span
                    className={`px-2 py-1 text-xs font-semibold rounded-full
                      ${ticket.supportTicketStatus === 'PENDING' ? 'bg-yellow-100 text-yellow-800'
